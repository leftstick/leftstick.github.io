...
下面是一些新手常见错误汇总，当你要开始自己的`angular2`旅程时，尽量避免吧。
...
第一眼看上面的例子，似乎基本就是`AngularJS 1`里的`ng-show`。其实不然，她们有着`!important`的不同。

`ng-show`和`ng-hide`都是通过一个叫`ng-hide`的CSS类来控制DOM元素的显示状态，`ng-hide`类就是简单的把元素设置成`display: none`。这里的关键在于，`AngularJS 1`在`ng-hide`类里增加了`!important`，用来调整该类的优先级，使得它能够覆盖其他任何地方对该元素的`display`属性操作。

再来说回本例，原生`hidden`属性上的`display: none`样式是由浏览器实现的。大多数浏览器是不会用`!important`来调整其优先级的。因此，通过`[hidden]="expression"`来控制元素显示状态很容易被其他样式意外的覆盖掉。超简单就可以加一个显示样式在你想隐藏的元素上。举个例子：如果我在其他地方对这个元素写了这样一个样式`display: flex`，这就比原生`hidden`属性的优先级高([看这里](https://jsbin.com/lozufoxexe/edit?html,css,output))。

基于这个原因，我们通常使用`*ngIf`切换元素是否存在来达成这一目的：
...
一个选择是，要求用户给他将要映射的列表项都加上预定义的`local variable`。这样的话，代码可以从上面例子改成这样：
...
* 如果在用view query(`@ViewChild`)时，结果集在视图初始化后可用。可以用`ngAfterViewInit`钩子
* 如果在用content query(`@ContentChildren`)，结果集在内容初始化后可用。可以用`ngAfterContentInit`钩子
...
不过需要注意的是，`ngOnChanges`当且仅当组件输入数据变化时被调用，指的是通过`@Input`装饰器显示指定那些数据。如果是`@ViewChildren`，`@ContentChildren`的结果集增加/删除了数据，`ngOnChanges`是不会被调用的。
...
在`angular2`里，我们介绍了一个新概念叫"structural directives"，用来描述那些根据表达式在`DOM`上或增加、或删除元素的指令。和其他指令不同，"structural directive"要么作用在template元素上、要么配合template属性、要么前缀"\*"。因为这个新语法特性，初学者常常犯错。
...
`angular2`将"in"换成"of"是为了和ES6中的`for-of`循环保持一致。也需要记住的是，如果不用"\*"语法糖，那么原始的repeater在`angular2`里要写作`ngForOf`，而非`ngForIn`
...
混着写是没必要的 － 而且事实上，这么写也不工作。当你用了语法糖(前缀"\*")以后，`angular2`就会把她当成一个模版属性，而不是一般的指令。具体来说，解析器拿到了`ngFor`后面的字符串，前缀指令的名字，然后当作模版属性来解析。如下代码：
...
从模版指令角度分析，发现模版指令后面就只有一个`ngFor`，别的什么都没了。那必然解析不会正确，也不会正常运行了。

如果从模版标签角度分析，他又缺了一个`ngFor`指令，所以也会报错。没了`ngFor`指令，`ngForOf`都不知道该对谁负责了。

那么可以这样修正，要么去掉""\*"语法糖，要么就完全按照"\*"语法糖简写方式书写
...
当我们通过"\*"语法糖简写代码时，我们必须遵守如下原则，以便解析器能够理解简写语法：

* 所有配置都要写在`*ngFor`的属性值里
* 通过`=`操作符设置`local variable`
* 通过`:`操作符设置输入属性
* 去掉输入属性里的`ngFor`前缀，譬如：`ngForOf`，就只写成`of`就可以了
* 用分号做分隔
